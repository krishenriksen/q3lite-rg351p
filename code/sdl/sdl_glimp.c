...
/*
===============
OpenGL ES compatibility - rg351p
===============
*/
...
#define GLE( ret, name, ... ) qgl##name = (name##proc *) SDL_GL_GetProcAddress("gl" #name); \
	...
	ri.Printf( PRINT_ALL, "Reported version from SDL2 (%s)\n\n\n\n\n\n", version );
	...
	if ( fixedFunction ) {
		...
		Com_Error(ERR_FATAL, "Unsupported: %s\n", version);
	}
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder, qboolean fixedFunction)
{
	...
	if (mode == -2)
	{
		...
		else
		{
			...
			glConfig.vidHeight = 320;
			ri.Printf( PRINT_ALL,
					"Cannot determine display resolution, assuming 480x320\n" );
		}
